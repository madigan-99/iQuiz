        ...
        GeometryReader {m in
            ...
            VStack {
                Spacer()
                    ...
                    Text("Question: " + current.question).font(.title)
                
                
                Text("Answer: " + current.answer).font(.title2)
                
                Text("Your Choice: " + selection).font(.title2)
                
                
                
                Text("Your answer is " + ((current.answer == selection) ? " right!" : " incorrect :(")).font(.title2)
                
                Spacer()
                HStack {
                    Spacer()
                    if (counter < IndivQuiz.quizContent.count) {
                        NavigationLink(destination:  Questions( numRight: numRight, counter: counter, quiz: IndivQuiz, current: IndivQuiz.quizContent[counter]).navigationBarHidden(true)) {
                            Spacer()
                            ZStack {
                        ...
                        NavigationLink(destination: Final(numRight: numRight, total: IndivQuiz.quizContent.count).navigationBarHidden(true)) {
                            Spacer()
                            ZStack {
                                RoundedRectangle(cornerRadius: 5).fill(Color.init(red: 0.66, green: 0.99, blue: 0.66)).frame(width: m.size.width / 1.2, height: m.size.height / 8, alignment: .center)
